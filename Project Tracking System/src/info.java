...
public class info {
    ...
    public ArrayList<String> getprojectLeaders() {
        ...
        try{
            ...
            String dbmsURL = "mysqle";
            String db = "death star project";
            String user = "darth vader hocam";
            String pwd = "regrets";
            String strConn = "jdbc:mysql://" + dbmsURL + "/" + db
                    + "?user=" + user + "&password=" + pwd;//bunu bulmak zor oldu //
            Connection conn = DriverManager.getConnection(strConn);
            ResultSet rs;
            try (Statement stmt1 = conn.createStatement()) {
                rs = stmt1.executeQuery("SELECT name FROM projectmembers WHERE role='Projectmanager';");
            }
            rs.beforeFirst();
            while (rs.next()) {
